...
// Start with a random memo.
let currentMemo = Math.floor(Math.random() * memos.length);
document.querySelector('#quote').textContent = memos[currentMemo];

// Generate a random memo, but never immediately repeat the current one.
document.querySelector('#newMemo').onclick = () => {
  let nextMemo;

  do {
    nextMemo = Math.floor(Math.random() * memos.length);
  } while (nextMemo === currentMemo && memos.length > 1);

  currentMemo = nextMemo;
  document.querySelector('#quote').textContent = memos[currentMemo];
};

// Virtual kiss delivery system.
